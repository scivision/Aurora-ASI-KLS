...
static void check_image_format(const boost::python::numpy::ndarray& input_image)
{
    ...
    if(input_image.get_dtype() != boost::python::numpy::dtype::get_builtin<unsigned char>())
        throw std::runtime_error("dtype of input_image must be uint8");

    if(!input_image.get_flags() & boost::python::numpy::ndarray::C_CONTIGUOUS)
        throw std::runtime_error("input_image must be C-style contiguous");
}

boost::python::tuple segment(const boost::python::numpy::ndarray& input_image, float sigma, float c, int min_size)
{
    ...
    boost::python::numpy::ndarray result_image = boost::python::numpy::empty(input_image.get_nd(), input_image.get_shape(), input_image.get_dtype());
    std::copy(seg_result_img->data, seg_result_img->data + w * h, reinterpret_cast<rgb*>(result_image.get_data()));

    delete seg_result_img;
    return boost::python::make_tuple<boost::python::numpy::ndarray, int>(result_image, num_css);
}

boost::python::tuple segment_label(const boost::python::numpy::ndarray& input_image, float sigma, float c, int min_size)
{
    check_image_format(input_image);
    ...
    boost::python::numpy::ndarray result_label = boost::python::numpy::empty(2, input_image.get_shape(), boost::python::numpy::dtype::get_builtin<int>());
    ...
    delete seg_result_img;
    return boost::python::make_tuple<boost::python::numpy::ndarray, int>(result_label, num_css);
}
